...
*/
		...
		public function play():Boolean {
			Logger.log("Logo Controller play start ... ");
			if (!_playlistReady) {
			...
			Logger.log("Logo Controller play _preplay ... "+_preplay);
			if (!_preplay) {
				_preplay = true;
			...
			Logger.log("Logo Controller play _model.playlist.currentItem ... "+(_model.playlist.currentItem));
			Logger.log("Logo Controller play _player.state ... "+(_player.state));
		...
		public function seek(pos:Number):Boolean {
			Logger.log("Logo Controller seek start ... ");
			Logger.log("Logo Controller seek pos = " + pos);
			if (locking) {
				return false;
			}
			...
			Logger.log("Logo Controller seek _model.media.state = " + (_model.media.state));
			switch (_model.media.state) {
				case PlayerState.PAUSED:
					play();
				case PlayerState.PLAYING:
				...
				case PlayerState.IDLE:
					...
					_idleSeek = pos;
					Logger.log("Logo Controller seek _preplay = " + (_preplay));
					if (!_preplay) {
						play();
					}
					return true;
		...
		private function bufferFullHandler(evt:MediaEvent):void {
			Logger.log("Logo controller bufferFullHandler _queuedSeek = " +　_queuedSeek);
			if (!locking) {
				if (_queuedSeek >= 0) {
					_model.media.seek(_queuedSeek);
					_queuedSeek = -1;

	...
	public dynamic class PlaylistItem {
		...
		public function PlaylistItem(obj:Object = null) {
			...
			for (var itm:String in obj) {
				Logger.log("Logo PlaylistItem PlaylistItem for itm = " + itm);
				if (itm == "levels") {
					if (!(obj[itm] is Array)) {
						continue;
					}
					Logger.log("Logo PlaylistItem PlaylistItem obj['levels'] is Array");
					var levels:Array = obj[itm] as Array;
					for (var i:Number = 0; i < levels.length; i++) {
						var level:Object = levels[i];
						Logger.log("Logo PlaylistItem PlaylistItem obj['levels'] " +
							"level['file'] = " + level['file'] + "; level['type'] = " + level['type'] + "; " +
							"level['default'] = " + level['default'] + "; level['streamer'] = " + level['streamer']);
						if (level['file']) {
					...
					for (i = 0; i < tracks.length; i++) {
						...
						Logger.log("Logo PlaylistItem PlaylistItem obj['tracks'] " +
							"track['file'] = " + track['file'] + "; track['kind'] = " + track['kind'] + "; " +
							"track['default'] = " + track['default'] + "; track['label'] = " + track['label']);
						if (track['file']) {

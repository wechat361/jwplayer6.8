...
//		protected var _removedButtons:Array = [];
		...
		public function ControlbarComponent(player:IPlayer) {
			...
			//Logger.log("Logo ControlbarComponent ControlbarComponent setTime ");
			setTime(0, 0);
			updateVolumeSlider();
		...
		*/
		...
		private function updateControlbarState():void {
			...
			//Logger.log("Logo ControlbarComponent updateControlbarState newLayout = " + newLayout);
			_currentLayout = removeInactive(newLayout);
			//Logger.log("Logo ControlbarComponent updateControlbarState _currentLayout = " + _currentLayout);
			Logger.log("Logo ControlbarComponent updateControlbarState player.state = " + player.state);
			if (player.state == PlayerState.PLAYING) {
				newLayout = newLayout.replace('play', 'pause');
				hideButton('play');
			} else if (player.state == PlayerState.IDLE) {
				Logger.log("Logo ControlbarComponent updateControlbarState PlayerState.IDLE _timeSlider = " + (_timeSlider==null));
				if (_timeSlider) {
					_timeSlider.reset();
					_timeSlider.thumbVisible = false;
				}
				if (_player.playlist.currentItem) {
					Logger.log("Logo ControlbarComponent updateControlbarState _player.playlist.currentItem.duration = " + (_player.playlist.currentItem.duration));
		...
		private var positionCount:Number = 0;
			...
			switch (evt.type) {
				...
				case MediaEvent.JWPLAYER_MEDIA_TIME:
					...
					/*if (evt.duration == -1 && evt.type == MediaEvent.JWPLAYER_MEDIA_TIME) {
						Logger.log("Logo ControlbarComponent mediaHandler if ");
						if (!_instreamMode) { 
							setText(player.playlist.currentItem.title || "Live broadcast"); 
						}
					} else {*/
						//Logger.log("Logo ControlbarComponent mediaHandler else ");
						if (scrubber) {
							//Logger.log("Logo ControlbarComponent mediaHandler if (scrubber) evt.duration = " + evt.duration);
							if (evt.duration <= -60) {
							...
							//Logger.log("Logo ControlbarComponent mediaHandler if (scrubber) evt.bufferPercent = " + evt.bufferPercent);
						...
						if (timeSlider) {
							//Logger.log("Logo ControlbarComponent mediaHandler if (timeSlider) ");
							timeSlider.setDuration(evt.duration);
							timeSlider.live = (evt.duration <= 0 && evt.duration > -60);
						}
						
						if (evt.position > 0 || evt.duration <= -60) { 
							Logger.log("Logo ControlbarComponent mediaHandler setTime evt.position = " + evt.position
											+ "; evt.duration = " + evt.duration);
							//当播放位置在同一位置停留次数超过100次时，自动停止加载      start
							if(prevPosition != evt.position) {
							...
							//当播放位置在同一位置停留次数超过100次时，自动停止加载      end
		...
		private function setTime(position:Number, duration:Number):void {
			...
			var timeSlider:TimeSlider = getSlider('time') as TimeSlider;
			//Logger.log("Logo ControlbarComponent setTime timeSlide = null ? " + (timeSlider == null));
			//Logger.log("Logo ControlbarComponent setTime duration ? " + (duration));
			if (timeSlider) {
		...
		private function levelsHandler(evt:MediaEvent):void {
			_levels = evt.levels;
			Logger.log("Logo ControlbarComponent levelsHandler .... _levels.length = " + (_levels.length));
			if (_levels.length > 1) {
		...
		private function levelChanged(evt:MediaEvent):void {
			Logger.log("Logo ControlbarComponent levelChanged ...._levels.length = " +(_levels.length) );
		...
		private function redraw():void {
			...
			if (_dispWidth > 0 && _dispWidth < 320) {
				...
				if (!_smallPlayer) {
					Logger.log("Logo ControlbarComponent redraw if (!_smallPlayer) setTime ");
					setTime(0,0);
					_smallPlayer = true;
				}
			} else {

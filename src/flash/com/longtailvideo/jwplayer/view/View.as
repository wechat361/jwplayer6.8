package com.longtailvideo.jwplayer.view {
	...
	public class View extends GlobalEventDispatcher {
		...
		private var _isDisplayLogo:Boolean = false;
		...
		public function redraw():void {
			...
			if (audioMode) {
				...
				////Logger.log("Logo View.as 422 _components.logo.show() start");
				//_components.logo.show();
				////Logger.log("Logo View.as 422 _components.logo.show() end");
				showMedia();
			}
...
//				var plug:IPlugin = _pluginsLayer.getChildAt(i) as IPlugin;
		...
		protected function itemHandler(evt:PlaylistEvent):void {
			//Logger.log("Logo itemHandler ----- ");
			//Logger.log("Logo itemHandler _model.playlist.currentItem ----- " + (_model.playlist.currentItem));
			//Logger.log("Logo itemHandler _model.playlist.currentItem.image ----- " + (_model.playlist.currentItem.image));
		...
		protected function loadImage(url:String):void {
			...
			//Logger.log("Logo loadImage url : " + url);
		...
		private function _imageTimerHandler(evt:TimerEvent):void {
			...
			//Logger.log("Logo loadImage _imageTimerHandler : ");
		...
		protected function imageComplete(evt:Event):void {
			//Logger.log("Logo loadImage imageComplete : ");
			if (_image) {
				_imageLoaded = true;
		...
		protected function showImage(evt:TimerEvent=null):void {
			//Logger.log("Logo loadImage showImage : ");
			if (!audioMode) {
//				_imageLayer.alpha = 0;
				_imageFade.fade(1);
		...
		protected function stateHandler(evt:PlayerStateEvent=null):void {
			//Logger.log("Logo stateHandler ----- ");
			imageDelay.reset();
			mediaDelay.reset();
			switch (_model.state) {
				case PlayerState.IDLE:
					//Logger.log("Logo stateHandler ----- PlayerState.IDLE");
					hideControls();
				...
				case PlayerState.BUFFERING:
					//Logger.log("Logo stateHandler ----- PlayerState.BUFFERING");
				...
				case PlayerState.PAUSED:
					//Logger.log("Logo stateHandler ----- PlayerState.PAUSED");
					showControls();
					Mouse.show();
					break;
				case PlayerState.PLAYING:
					//Logger.log("Logo stateHandler ----- PlayerState.PLAYING");
					hideControls();
					mediaDelay.start();
					break;
			}
		...
		protected function completeHandler(evt:PlaylistEvent):void {
			//Logger.log("Logo completeHandler ----- ");
			_completeState = true;
			fullscreen(false);
		...
		public function destroyInstream():void {
			//Logger.log("Logo destroyInstream ----- ");
		...
		protected function instreamAnimationComplete(evt:Event):void {
			//Logger.log("Logo instreamAnimationComplete ----- ");
		...
		protected function get audioMode():Boolean {
			//Logger.log("Logo audioMode ----- ");
			var plConfig:PluginConfig = _model.config.pluginConfig('playlist');
			////Logger.log("Logo View.as audioMode plConfig = 1");
		...
		private function showControls():void {
			...
			/*if (!audioMode) {
				//Logger.log("Logo View.as 862 _components.logo.show() start");
				//_components.logo.show()

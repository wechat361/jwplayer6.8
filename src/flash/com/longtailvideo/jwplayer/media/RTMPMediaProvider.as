	...
	**/
	public class RTMPMediaProvider extends MediaProvider {
		...
		override public function load(itm:PlaylistItem):void {
			Logger.log("Logo RTMPMediaProvider load ......");
			_item = itm;
			_position = 0;
			// Set Video or StageVideo
			Logger.log("Logo RTMPMediaProvider load !_video = " + (!_video));
			if(!_video) {
			...
			Logger.log("Logo RTMPMediaProvider load _item.file = " + (_item.file));
			if (_item.file.substr(0,4) == 'rtmp') {
				...
				Logger.log("Logo RTMPMediaProvider load _levels = " + _levels);
				loadWrap();
		...
		private function loadWrap():void {
			Logger.log("Logo RTMPMediaProvider loadWrap _type = " + _type);
			...
			var level:Number = 0;
			Logger.log("Logo RTMPMediaProvider loadWrap _config.qualitylabel = " + (_config.qualitylabel));
			...
			try {
				Logger.log("Logo RTMPMediaProvider loadWrap _connection.connect(_application) = " + (_application));
		...
		public function onClientData(data:Object):void {
			Logger.log("Logo RTMPMediaProvider onClientData data.type = " + (data.type));
			switch (data.type) {
				...
				case 'metadata':
					if(!_metadata) {
						...
						_metadata = true;
						Logger.log("Logo RTMPMediaProvider onClientData data.duration = " + (data.duration));
						if (data.duration) {
				...
				case 'complete':
					Logger.log("Logo RTMPMediaProvider onClientData complete item.duration = " + _item.duration);
		...
		override public function seek(pos:Number):void {
			if(_item.duration > 0) {
				...
				{
					Logger.log("Logo RTMPMediaProvider seek error.info.code = " + (error.info.code));
					if(error.info.code == "NetStream.Seek.InvalidTime") {
						_stream.seek(error.info.details);
					}
				}
		...
		private function setStream():void {
			Logger.log("Logo RTMPMediaProvider setStream _connection = " + _connection);
			...
			Logger.log("Logo RTMPMediaProvider setStream getConfigProperty('bufferlength') = " + getConfigProperty('bufferlength'));
			...
			Logger.log("Logo RTMPMediaProvider setStream _stage = " + _stage);
			if(_stage) {
				_stage.attachNetStream(_stream);
			} else {
				_video.attachNetStream(_stream);
			}
			Logger.log("Logo RTMPMediaProvider setStream config.mute = " + config.mute + "; config.volume = " + config.volume);
